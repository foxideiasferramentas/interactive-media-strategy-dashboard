...
import { Search, Globe, Target, TrendingUp, Plus, X, Library, Search as SearchIcon, Users, BookmarkCheck, Pencil, Trash2 } from "lucide-react";
...
import type { GoogleAudience, GoogleCreative, SavedAudience, SavedCreative } from "../data/types";
...
function LibraryPickerModal({
  ...
  onPick: (audience: GoogleAudience) => void;
  onClose: () => void;
}) {
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="px-6 pt-4">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar público na biblioteca..." className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500" autoFocus />
          </div>
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((saved) => (
              <button key={saved.id} onClick={() => {
                const isCrossPlatform = saved.type !== "google";
                const fresh: GoogleAudience = {
                  ...saved.audience as GoogleAudience,
                  id: `aud-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`,
                  creatives: isCrossPlatform ? [] : (saved.audience.creatives as GoogleCreative[]).map((c) => ({ ...c, id: `cr-${Date.now()}-${Math.random().toString(36).slice(2, 5)}` })),
                };
                onPick(fresh);
                onClose();
...
function AudienceEditModal({
  ...
  onSave: (updated: GoogleAudience) => void;
  onClose: () => void;
}) {
  const [draft, setDraft] = useState<GoogleAudience>({ ...audience });

  const field = (key: keyof GoogleAudience) => (
    <input
      type="text"
      value={(draft[key] as string) ?? ""}
      onChange={(e) => setDraft((d) => ({ ...d, [key]: e.target.value }))}
      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500"
    />
  );

  const textarea = (key: keyof GoogleAudience) => (
    <textarea
      value={(draft[key] as string) ?? ""}
      onChange={(e) => setDraft((d) => ({ ...d, [key]: e.target.value }))}
      rows={3}
      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500 resize-none"
    />
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden max-h-[90vh] flex flex-col">
        <div className="px-6 py-5 border-b border-slate-200 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Google Ads</p>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 flex-shrink-0">
          ...
          <button onClick={() => { onSave(draft); onClose(); }} className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl transition-colors">Salvar</button>
        </div>
      </motion.div>
    </div>
  );
...
function CreativePickerModal({
  ...
  onPick: (creative: GoogleCreative) => void;
  onClose: () => void;
}) {
  const [search, setSearch] = useState("");
  const filtered = savedCreatives.filter(
    (s) => s.platform === "google" && s.label.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Google Ads · {filtered.length} criativo{filtered.length !== 1 ? "s" : ""} salvos</p>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors"><X className="w-5 h-5" /></button>
        </div>
        <div className="px-6 pt-4">
          <div className="relative">
            <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar criativo..." className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500" autoFocus />
          </div>
        </div>
        <div className="px-6 py-4 max-h-96 overflow-y-auto space-y-2">
          {filtered.length === 0 ? (
            <div className="text-center py-12">
              <Library className="w-8 h-8 text-slate-200 mx-auto mb-3" />
              <p className="text-sm text-slate-400 font-medium">{savedCreatives.filter((s) => s.platform === "google").length === 0 ? "Nenhum criativo salvo ainda." : "Nenhum resultado para essa busca."}</p>
            </div>
          ) : (
            filtered.map((saved) => (
              <button key={saved.id} onClick={() => {
                onPick({ ...saved.creative as GoogleCreative, id: `cr-${Date.now()}-${Math.random().toString(36).slice(2, 5)}` });
                onClose();
              }} className="w-full flex items-start gap-3 p-4 rounded-xl border border-slate-200 bg-slate-50 hover:border-emerald-300 hover:bg-emerald-50/50 transition-all text-left group">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 bg-emerald-100 text-emerald-600">
                  <BookmarkCheck className="w-3.5 h-3.5" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-slate-900 truncate group-hover:text-emerald-700">{saved.label}</p>
                ...
                <span className="text-xs text-emerald-500 font-bold opacity-0 group-hover:opacity-100 transition-opacity mt-1 flex-shrink-0">Usar</span>
              </button>
            ))
          )}
        </div>
...
export function GoogleAds() {
  ...
  const [editingAudience, setEditingAudience] = useState<GoogleAudience | null>(null);

  const addAudience = () => {
    if (!campaign) return;
    const newAud: GoogleAudience = { id: uid(), title: "Novo Público", description: "", tag: "Segmentação", creatives: [] };
    updateCampaign({ ...campaign, google: { ...campaign.google, [activeStep]: [...campaign.google[activeStep], newAud] } });
    setSelectedAudienceIndex(campaign.google[activeStep].length);
  };

  const updateAudience = (updated: GoogleAudience) => {
    if (!campaign) return;
    updateCampaign({ ...campaign, google: { ...campaign.google, [activeStep]: campaign.google[activeStep].map(a => a.id === updated.id ? updated : a) } });
  };

  const removeAudience = (id: string) => {
    if (!campaign) return;
    updateCampaign({ ...campaign, google: { ...campaign.google, [activeStep]: campaign.google[activeStep].filter(a => a.id !== id) } });
  ...
  const removeCreative = (id: string) => {
    ...
    updateCampaign({ ...campaign, google: { ...campaign.google, [activeStep]: campaign.google[activeStep].map((a) => a.id === updatedAud.id ? updatedAud : a) } });
  };

  const addAudienceFromLibrary = (aud: GoogleAudience) => {
    if (!campaign) return;
    updateCampaign({ ...campaign, google: { ...campaign.google, [activeStep]: [...campaign.google[activeStep], aud] } });
    setSelectedAudienceIndex(campaign.google[activeStep].length);
  };

  const addCreativeFromLibrary = (creative: GoogleCreative) => {
    if (!campaign || !selectedAudience) return;
                              ...
                              {selectedAudience.creatives.flatMap((creative) => {
                                ...
                                <div key={c.displayId} className="break-inside-avoid mb-4">
                                  ...
                                  >
                                    {isAuthenticated && (
                                      <button
                                        onClick={(e) => { e.stopPropagation(); removeCreative(c.id); }}
                                    ...
                                    <CreativeCard
                                      ...
                                      globalSitelinks={campaign.google.sitelinks}
                                      globalSnippets={campaign.google.structuredSnippets}
                                      globalCall={campaign.google.callExtension}
                                    />
                                  </motion.div>
                                </div>
                              ))}
    ...
    >
      {isAdmin && (
        <div className="absolute top-3 right-3 flex items-center gap-1 opacity-0 group-hover/aud-card:opacity-100 transition-opacity">
          <button onClick={onEdit} className="p-1 rounded-lg bg-white border border-gray-200 text-gray-400 hover:text-emerald-600 hover:border-emerald-300 transition-colors shadow-sm"><Pencil className="w-3 h-3" /></button>
